...
Renderer::Renderer(Window& parent) : OGLRenderer(parent) {
	...
	heightMap = new HeightMap(TEXTUREDIR"noise.png", false);
	...
	for (int i = 0; i < LIGHT_NUM; i++)
	{
		...
		l.SetRadius(2500 + rand() % 250);
	}
	sceneShader = new Shader("bumpVertex.glsl", "bumpFragment.glsl");
	...
	if (!pointlightShader->LoadSuccess() || !combineShader->LoadSuccess() || !sceneShader->LoadSuccess()) {
		return;
	}
