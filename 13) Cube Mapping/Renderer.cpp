...
Renderer::Renderer(Window& parent) : OGLRenderer(parent) {
	...
	reflectShader = new Shader("reflectVertex.glsl", "reflectFragmet.glsl");
	skyboxShader = new Shader("skyboxVertex.glsl", "skyboxFragmet.glsl");
	lightShader = new Shader("perPixelVertex.glsl", "perPixelFragmet.glsl");
	if (!reflectShader->LoadSuccess() || !skyboxShader->LoadSuccess() || !lightShader->LoadSuccess()) {
		return;
	}

	Vector3 heightmapSize = heightMap->GetHeightMapSize();
	camera = new Camera(-45, 0, heightmapSize * Vector3(0.5, 5, 0.5));
	light = new Light(heightmapSize * Vector3(0.5, 1.5, 0.5), Vector4(1, 1, 1, 1), Vector4(1, 1, 1, 1), heightmapSize.x);
...
void Renderer::DrawWater(){
	...
	glUniform1i(glGetUniformLocation(lightShader->GetProgram(), "diffuseTex"), 2);
